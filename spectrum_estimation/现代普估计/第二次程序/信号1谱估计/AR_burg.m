...
orderpredict=20;                     %模型阶数
 for i=1:50
    ...
    p{i}=pburg(y1{i},orderpredict);                 %用burg算法估计功率谱
%     plot(w,p{i});
 end
p1=zeros(nfft,1);
for i=1:50
...
ylabel('功率/db');
grid on;
title(['p=',num2str(orderpredict),';N=',num2str(N),':snr1=',num2str(snr1),';snr2=',num2str(snr2),';50次Burg法功率谱估计均值']);
hold on;
...
grid on;
title(['p=',num2str(orderpredict),';N=',num2str(N),':snr1=',num2str(snr1),';snr2=',num2str(snr2),';50次Burg法功率谱估计方差']);
xlabel('归一化频率/Hz');
ylabel('方差/db');